# **Zero to Pentesting Hero: A 3-Month Hands-On Journey**  

## **📅 Phase 1: Foundations (Weeks 1-4)**  
### **Week 1 (June 5-11)**  
| **Day** | **Topic**            | **Tasks**                          | **Resources**                                                                 |  Done     |
|---------|----------------------|------------------------------------|------------------------------------------------------------------------------|------------------|
| 1-3     | Networking Basics    | Study protocols, Nmap basics      | [NetworkChuck's Course](https://youtu.be/qiQR5rTSshw)                       |           ✅
|         |                      | Practice scanning                 | [Nmap Cheat Sheet](https://www.stationx.net/nmap-cheat-sheet/)               |
| 4-6     | Linux & Bash         | Complete Bandit Wargame           | [OverTheWire Bandit](https://overthewire.org/wargames/bandit/)               |
|         |                      | Learn Linux commands              | [Linux Command Guide](https://linuxjourney.com/)                             |
| 7       | Security Fundamentals| Review OWASP Top 10               | [OWASP Top 10](https://owasp.org/www-project-top-ten/)                       |
|         |                      | Complete Pre-Security path        | [TryHackMe Pre-Security](https://tryhackme.com/path/outline/presecurity)     |

---

### **Week 2 (June 12-18)**  
| **Day** | **Topic**            | **Tasks**                          | **Resources**                                                                 |
|---------|----------------------|------------------------------------|------------------------------------------------------------------------------|
| 8-10    | Wireshark & PCAPs    | Analyze traffic captures          | [Wireshark Docs](https://www.wireshark.org/docs/)                            |
|         |                      | Solve PCAP challenges             | [PCAP Challenges](https://www.malware-traffic-analysis.net/)                 |
| 11-14   | Kali Setup           | Install Kali Linux                | [Kali Installation Guide](https://www.kali.org/docs/installation/)           |
|         |                      | Explore Kali tools                | [Kali Tools List](https://www.kali.org/tools/)                               |

---

### **Week 3 (June 19-25)**  
| **Day** | **Topic**            | **Tasks**                          | **Resources**                                                                 |
|---------|----------------------|------------------------------------|------------------------------------------------------------------------------|
| 15-18   | Nmap Deep Dive       | Master advanced scanning          | [Nmap Network Scanning (Book)](https://nmap.org/book/)                       |
|         |                      | Practice service detection        | [HackTricks Nmap Guide](https://book.hacktricks.xyz/network-services-pentesting/nmap) |
| 19-21   | Web App Basics       | Complete PortSwigger Labs         | [PortSwigger Labs](https://portswigger.net/web-security)                     |
|         |                      | Learn Burp Suite                  | [Burp Suite Tutorial](https://www.youtube.com/watch?v=G3hpAeoZ4ek)           |

---

### **Week 4 (June 26 - July 2)**  
| **Day** | **Topic**            | **Tasks**                          | **Resources**                                                                 |
|---------|----------------------|------------------------------------|------------------------------------------------------------------------------|
| 22-25   | Password Cracking    | Practice with Hashcat             | [Hashcat Examples](https://hashcat.net/wiki/doku.php?id=example_hashes)      |
|         |                      | Crack RockYou.txt hashes          | [RockYou.txt Practice](https://github.com/brannondorsey/naive-hashcat)       |
| 26-28   | Metasploit Intro     | Study Metasploit modules          | [Metasploit Unleashed](https://www.offensive-security.com/metasploit-unleashed/) |
|         |                      | Exploit VulnServer                | [VulnServer Exploit](https://github.com/stephenbradshaw/vulnserver)          |

---

## **💥 Phase 2: Hands-On Hacking (Weeks 5-8)**  
### **Week 5 (July 3-9)**  
| **Day** | **Topic**            | **Tasks**                          | **Resources**                                                                 |
|---------|----------------------|------------------------------------|------------------------------------------------------------------------------|
| 29-32   | Burp Suite           | Complete Burp Suite Academy        | [Burp Suite Academy](https://portswigger.net/web-security)                   |
|         |                      | Practice on WebSec Academy         | [WebSec Academy](https://websecacademy.com/)                                 |
| 33-35   | Metasploitable 2     | Exploit Metasploitable VM          | [Metasploitable Guide](https://metasploit.help.rapid7.com/docs/metasploitable-2) |
|         |                      | Search Exploit-DB for vulns        | [Exploit Database](https://www.exploit-db.com/)                              |

---

### **Week 6 (July 10-16)**  
| **Day** | **Topic**            | **Tasks**                          | **Resources**                                                                 |
|---------|----------------------|------------------------------------|------------------------------------------------------------------------------|
| 36-39   | Privilege Escalation | Study GTFOBins techniques          | [GTFOBins](https://gtfobins.github.io/)                                      |
|         |                      | Run WinPEAS/LinPEAS                | [WinPEAS](https://github.com/carlospolop/PEASS-ng)                          |
| 40-42   | HTB Machines         | Complete HTB Starting Point        | [HTB Starting Point](https://www.hackthebox.com/starting-point)              |
|         |                      | Follow TJ_Null’s list              | [TJ_Null’s HTB List](https://docs.google.com/spreadsheets/d/1dwSMIAPIam0PuRBkCiDI88pU3yzrqqHkDtBngUHNCw8/edit#gid=0) |

---

### **Week 7 (July 17-23)**  
| **Day** | **Topic**            | **Tasks**                          | **Resources**                                                                 |
|---------|----------------------|------------------------------------|------------------------------------------------------------------------------|
| 43-46   | Active Directory     | Study AD attacks                   | [HTB AD Walkthroughs](https://www.hackthebox.com/blog/tag/Active+Directory)  |
|         |                      | Practice with Impacket             | [Impacket Cheat Sheet](https://www.secureauth.com/labs/open-source-tools/impacket) |
| 47-49   | BloodHound           | Map AD environments                | [BloodHound Docs](https://bloodhound.readthedocs.io/)                        |
|         |                      | Simulate attacks                   | [AD Attack Simulation](https://github.com/BloodHoundAD/BloodHound)           |

---

### **Week 8 (July 24-30)**  
| **Day** | **Topic**            | **Tasks**                          | **Resources**                                                                 |
|---------|----------------------|------------------------------------|------------------------------------------------------------------------------|
| 50-53   | VulnHub              | Root VulnHub machines              | [VulnHub Machines](https://www.vulnhub.com/)                                 |
|         |                      | Follow Kioptrix walkthrough        | [Kioptrix Walkthrough](https://www.hackingarticles.in/kioptrix-level-1-walkthrough/) |
| 54-56   | CTFs                 | Complete TryHackMe Offensive Path  | [TryHackMe Offensive Path](https://tryhackme.com/path/outline/offensivepentesting) |
|         |                      | Join CTF competitions              | [CTFtime](https://ctftime.org/)                                              |

---

## **🔥 Phase 3: Advanced Practice (Weeks 9-12)**  
### **Week 9 (July 31 - August 6)**  
| **Day** | **Topic**            | **Tasks**                          | **Resources**                                                                 |
|---------|----------------------|------------------------------------|------------------------------------------------------------------------------|
| 57-60   | Kerberos Attacks     | Learn Kerberoasting                | [Kerberoasting Guide](https://www.ired.team/offensive-security-experiments/active-directory-kerberos-abuse/t1208-kerberoasting) |
|         |                      | Exploit HTB Heist                  | [HTB Heist](https://www.hackthebox.com/achievement/machine/573859/377)       |
| 61-63   | Python Scripting     | Write pentesting scripts           | [Python for Pentesters](https://github.com/dloss/python-pentest-tools)       |
|         |                      | Study Black Hat Python             | [Black Hat Python (Book)](https://nostarch.com/blackhatpython)               |

---

### **Week 10 (August 7-13)**  
| **Day** | **Topic**            | **Tasks**                          | **Resources**                                                                 |
|---------|----------------------|------------------------------------|------------------------------------------------------------------------------|
| 64-67   | Buffer Overflows     | Practice stack-based overflows     | [Buffer Overflow Guide](https://www.thegreycorner.com/p/beginning-stack-based-buffer.html) |
|         |                      | Exploit HTB Gatekeeper             | [HTB Gatekeeper](https://www.hackthebox.com/achievement/machine/1586/377)   |
| 68-70   | HTB/VulnHub         | Root medium-difficulty machines    | [Medium HTB Machines](https://www.hackthebox.com/home/machines/rating/4)    |
|         |                      | Practice VulnHub Medium            | [VulnHub Medium](https://www.vulnhub.com/?q=medium&sort=date)               |

---

### **Week 11 (August 14-20)**
| **Day** | **Topic**            | **Tasks**                          | **Resources**                                                                 |
|---------|----------------------|------------------------------------|------------------------------------------------------------------------------|
| 71-74   | Bug Bounty           | Read HackerOne reports             | [HackerOne Reports](https://github.com/reddelexc/hackerone-reports)          |
|         |                      | Study bug bounty tactics           | [Bug Bounty Playbook](https://github.com/arkadiyt/bounty-targets-data)       |
| 75-77   | Report Writing       | Review sample reports              | [Sample Pentest Report](https://www.offensive-security.com/reports/sample-penetration-testing-report.pdf) |
|         |                      | Use reporting templates            | [Vuln Reporting Template](https://github.com/devanshbatham/Awesome-Bugbounty-Writeups) |

---

### **Week 12 (August 21-26)**  
| **Day** | **Topic**            | **Tasks**                          | **Resources**                                                                 |
|---------|----------------------|------------------------------------|------------------------------------------------------------------------------|
| 78-81   | Final Challenge      | Attack HTB RastaLabs               | [HTB RastaLabs](https://www.hackthebox.com/home/labs/rastalabs)             |
|         |                      | Complete VulnHub Final Exam        | [VulnHub Final Exam](https://www.vulnhub.com/entry/finalexam-1,249/)        |
| 82-84   | Cert Prep            | Study eJPT syllabus                | [eJPT Syllabus](https://elearnsecurity.com/ejpt-certification/)              |
|         |                      | Review PNPT resources              | [PNPT Resources](https://www.tryhackme.com/path/outline/pnpt)               |

---

## **💡 Pro Tips**  
✔ **Join Discord** (HTB, TryHackMe, NetSec) for help  
✔ **Write Blog Posts** (Medium/Dev.to) to reinforce learning  
✔ **Build a Home Lab** (Raspberry Pi + vulnerable VMs)  

---


**"The best way to learn hacking is to break things... then fix them."** 
---  

🔐 **License:** [MIT](https://opensource.org/licenses/MIT) | **Author:** [alex](https://github.com/stilla1ex)  

*(This roadmap is aggressive—adjust based on your pace!)*  

---

Happy Hacking! 🚀💻🔥
